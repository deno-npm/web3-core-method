...
import {
  ...
  helpers,
  promiEvent,
  Subscriptions as SubscriptionsModule,
  utils,
} from "./deps.js";
...
const { subscriptions: Subscriptions } = SubscriptionsModule;
...
var Method = function Method(options) {
  ...
  this.transactionConfirmationBlocks = options.transactionConfirmationBlocks ||
    24;
...
Method.prototype._confirmTransaction = function (defer, result, payload) {
  var method = this,
    ...
    lastBlock = null,
    receiptJSON = "",
    gasProvided = (_.isObject(payload.params[0]) && payload.params[0].gas)
    ...
    if (!err) {
      ...
      return (existingReceipt
        ...
        .then(async function (receipt) {
          ...
          if (
            method.extraFormatters && method.extraFormatters.receiptFormatter
          ) {
          ...
          if (defer.eventEmitter.listeners("confirmation").length > 0) {
            ...
            if (existingReceipt === undefined || confirmationCount !== 0) {
              ...
              if (isPolling) { // Check if actually a new block is existing on polling
                if (lastBlock) {
                  block = await _ethereumCall.getBlockByNumber(
                    lastBlock.number + 1,
                  );
                  if (block) {
                    lastBlock = block;
            ...
            if (
              confirmationCount === method.transactionConfirmationBlocks + 1
                ...
                try {
                  ...
                  if (
                    ...
                    (method.call === "eth_sendTransaction" ||
                      method.call === "eth_sendRawTransaction")
                  ) {
                    ...
                    if (method.call === "eth_sendRawTransaction") {
                      ...
                      var parsedTx = EthersTransactionUtils.parse(
                        rawTransactionHex,
                      );
                    ...
                    if (revertMessage) { // Only throw a revert error if a revert reason is existing
                      utils._fireError(
                        errors.TransactionRevertInstructionError(
...
Method.prototype.buildCall = function () {
  var method = this,
    isSendTx = (method.call === "eth_sendTransaction" ||
  ...
  var send = function () {
    ...
    var sendTxCallback = function (err, result) {
      if (method.handleRevert && isCall && method.abiCoder) {
        ...
        if (reasonData) {
          var reason = method.abiCoder.decodeParameter(
            "string",
            "0x" + reasonData,
          );
    ...
    var sendRequest = function (payload, method) {
      if (
        method && method.accounts && method.accounts.wallet &&
        ...
        if (payload.method === "eth_sendTransaction") {
          ...
          if (wallet && wallet.privateKey) {
            ...
            return method.accounts.signTransaction(txOptions, wallet.privateKey)
              ...
              .catch(function (err) {
                if (
                  _.isFunction(defer.eventEmitter.listeners) &&
    ...
    if (
      isSendTx && _.isObject(payload.params[0]) &&
...
Method.prototype.getRevertReason = function (txOptions, blockNumber) {
  ...
  return new Promise(function (resolve, reject) {
    (new Method({
      ...
      .createFunction(self.requestManager)(
        txOptions,
...
Method.prototype.isRevertReasonString = function (data) {
  return _.isString(data) && ((data.length - 2) / 2) % 32 === 4 &&
    data.substring(0, 10) === "0x08c379a0";
};
